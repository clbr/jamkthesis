...
nodes could 
not perform well both the extreme low end (64 MB and 128 MB VRAM sizes) and at the more 
common sizes. As the common graphics cards for sale are approaching 2 GB, and the wide 
...
pressure under higher VRAM amounts.
...
.Visual scores
image::pics/pts3.png[scaledwidth="95%"]
...
as that test was equal under the simulation.
...
highest peak coincides with the comparison target, but it seems to be fairly rare. So for 
...
/reduction/ in score. As the number of writes increased, the score started to increase 
again. It is exactly this kind of new, unexpected connections that neural networks excel at, 
and which humans may never find.

The results fell a bit short of expectations. Given the difference to Catalyst, I hoped for 
...
better solution. Second, it is possible the input parameters are adequate, but the 
processing power of the network is not. If so, it would need more hidden nodes, which would 
also mean slower training.
...
I would like to thank Jerome Glisse for his guidance, Thomas Hellstrom for reviews, and 
everyone who contributed data.
