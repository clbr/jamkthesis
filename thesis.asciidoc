...
Under the graphics context, compaction is still doable, but ideally would not be done while 
a heavy workload is running. The delay caused by a VRAM
memory compaction operation may be measured in milliseconds, which
may cause the frame to take too long, and would be unacceptable stutter to the user.
...
In total, the results fell a bit short of expectations. Given the difference to Catalyst, I 
