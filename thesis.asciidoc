...
.Hyperbolic tangent and smootherstep, scaled to use the same input/output
...
smootherstep function, both in floating point, and converted to use fixed-point mathematics.
...
Each function was ran 10^9 times. Surprisingly, the fixed point implementation was the 
slowest of all. A single *tanh()* call took approximately 12.6 ns. A single fixed-point 
*smootherstep()* call took ~13.5 ns. The fastest of all, floating-point *smootherstep()*, 
took only ~8 ns per call.
